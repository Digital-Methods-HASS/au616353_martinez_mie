---
title: "Assignment W4 - Compute Tree Canopy"
author: "Mie Arnau Martinez"
date: "22/02/2021 updated `r format(Sys.time(), '%B %d, %Y')`" 
output:
...
head(neighborhoods_counts) # it does
...
# saving the shapefile for neighborhoods (need this for knitting the html)
st_write(neighborhoods_avg_canopy, dsn = "data/n_avg_canopy.shp", driver = "ESRI Shapefile")
```
...
neighborhoods$avg_canopy <- as.numeric(neighborhoods$avg_canopy) # Change class to numeric (before it was an array)
tm_shape(neighborhoods) + 
...
tm_shape(manhattan) + 
  tm_rgb() # using tm_rgb instead of tm_raster as it combines the multiband raster
```
...
# dependencies for knit
library(tmap)
library(raster)
library(sf)

manhattan <- brick("data/manhattan.tif")
neighborhoods <- st_read("data/n_avg_canopy.shp")

# Creating map1 with manhattan and neighborhoods
map1 <- tm_shape(manhattan) + 
  tm_rgb() +
  tm_shape(neighborhoods) + 
  tm_borders(col = "black", lwd = 1, alpha = 0.5)

# Creating map2 with neighborhoods and tree density
map2 <- tm_shape(neighborhoods, bbox = manhattan) + # adding a manhattan bbox
    tm_polygons("tr_dnst", palette = "Greens", 
        style = "quantile", n = 7, 
        title = "Trees per sq. KM") 

# Creating map3 with neighborhoods and average tree canopy
map3 <- tm_shape(neighborhoods, bbox = manhattan) +
    tm_polygons("avg_cnp", palette = "Greens", 
        style = "quantile", n = 7, 
        title = "Average tree canopy (%)") 
  
# Displaying all maps side by side using tmap_arrange
tmap_arrange(map1, map2, map3, asp = NA)

```
As can be seen from the three maps, average tree canopy is a better representation of the aerial map compared to the tree density measure.
