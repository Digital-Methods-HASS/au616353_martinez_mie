...
# Converting the table into an sf object on the basis of X and Y columns
cities_sf <- st_as_sf(cities, coords = c("Longitude (X)", "Latitude (Y)"))

# Defining the projection of Lat/Long coordinates as EPSG 4326
cities_sf4326<- st_set_crs(cities_sf, 4326)

# Transforming the projection to a 2D projection using EPSG 3035
cities_sf3035<- st_transform(cities_sf4326, crs = 3035)

# Verifying the projection is 'projected' not 'geographic'
st_crs(cities_sf3035) # From the output we can see that it is projected (PROJCRS)
...
```
The measurement units is in metres.
...
```{r}
# Making buffer of 5 km. Check the units of your object to correctly assign value to dist
...
tm_shape(ten_buffers)  +
  ...
  tm_shape(ten_cities) +
  tm_text("Ancient Toponym", size = 1, auto.placement = 5) + # changed the size of the text to 1
  tm_dots(col = "#1c9099", # added color from colorbrewer2.org
             size = 0.1) +
  tm_scale_bar(breaks = c(0, 10, 20), # defining the scale bar breaks
               text.size = 10,
               position = c("left", "bottom")) +
  tm_compass(position = c("right", "top"),
             type = "rose", 
             size = 2) +
  tm_credits(position = c("left", "bottom"), # added position left bottom
             text = "Mie Arnau Martinez") + # added credits text
  tm_layout(main.title = "Ancient cities",
            bg.color = "beige",
...
class(inscriptions$coordinates)
...
# Removing the "c(" and ")" from the two new variables
i_sm$latitude <- gsub(")", "", i_sm$latitude)
i_sm$longitude <- gsub("[c(]", "", i_sm$longitude)
...
```{r}
# Wrangling full data set of inscriptions
i <- inscriptions %>% 
...
i$longitude <- gsub("[c(]", "", i$longitude) %>% 
  as.numeric(i$longitude)

# Removing the ")" from the latitude values and making them numeric
i$latitude <- gsub("[)]", "", i$latitude) %>% 
  as.numeric(i$latitude)

# R is messing with me so it doesn't allow me to filter both criteria in one line.
i <- i %>% 
  filter(i$longitude > 5) 
i <- i %>% 
  filter(i$longitude < 20)
...
crs_cities <- st_crs(cities_sf3035) # defining the crs
insc_sf3035 <- st_transform(insc_sf4326, crs = crs_cities) # transforming the crs

# Create a convex hull around the inscriptions's points dissolved into a MULTIPOINT
insc_ch <- st_convex_hull(st_union(insc_sf3035)) # Making it a multipoint by using st_union

# Create a buffer from a cluster of cities that fall within the convex hull 
st_crs(cities_5km) # Checking the crs of cities_5km
cities_it <- st_intersection(insc_ch, st_transform(cities_5km, crs = crs_cities)) # transforming the crs of cities_5km and calculating the intersection with insc_ch
...
```
So, there are 6.5 times more inscriptions in the cities compared to the rural areas in Italy.
