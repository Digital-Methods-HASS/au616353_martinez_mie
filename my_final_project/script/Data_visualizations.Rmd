...
library(pacman)
p_load(tidyverse, stringr, rgeos, tm)
p_load(ggplot2, rnaturalearth, rnaturalearthdata, ggrepel, ggspatial, sf)
#p_load(cowplot, googleway, libwgeom)
...
world_points<- st_point_on_surface(world) # This finds a point on each country (polygon)
world_points <- cbind(world, st_coordinates(st_point_on_surface(world$geometry))) # This ties the coordinates to a country so that one can print the countries' names on the map.
...
gdp_2020 <- as.data.frame(gdp_2020) # Making this into a data frame as to sf objects can't be merged
